 ...
 *
 * @tparam C the type to decode into
 */
trait ConfigDecoder[C] extends ConfigLoader[C]:
  /**
   * Decodes a raw string value into a typed value.
   *
   * @param raw the raw string value to decode
   * @param ctx the decoding context containing annotations and field path
   * @return either a configuration error or the decoded value
   */
   ...
   *
   * @param f the function to apply to the decoded value
   ...
   *
   * @param f the function to apply that may fail with a ConfigError
   * @tparam B the target type
   * @return a new decoder for type B
   */
  def emap[B](
