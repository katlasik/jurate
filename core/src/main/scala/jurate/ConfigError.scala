...
/** Represents an error where a required configuration value was not found.
  *
  * @param fieldPath
  *   the path to the field that was missing
  * @param annotations
  *   the annotations that were tried to find the value
  */
...
/** Represents an error where a configuration value was found but could not be
  * decoded.
  *
  * @param receivedValue
  *   the raw string value that was received
  * @param detail
  *   additional details about why decoding failed
  * @param fieldPath
  *   the path to the field that failed to decode
  * @param annotation
...
/** Represents other configuration errors.
  ...
  * @param annotation
  *   optionally the annotation related to the error
  */
case class Other(
    fieldPath: FieldPath,
    detail: String,
...
/** Configuration loading error containing one or more error reasons.
  *
  * @param reasons
  *   the list of error reasons that caused this configuration error
  */
case class ConfigError(reasons: List[ConfigErrorReason])
  ...
  /** Combines this error with another error, merging their reasons.
    *
    * @param other
    *   the other error to combine with
    * @return
    *   a new ConfigError containing all reasons from both errors
    */
  ...
  /** Formats this error using the provided error printer.
    *
    * @param printer
    *   the error printer to use for formatting
    * @return
...
object ConfigError {
  ...
  /** Creates a configuration error for an invalid value.
    ...
    *   the annotation that provided the value
    * @return
    *   a ConfigError with an Invalid reason
    */
  def invalid(
      fieldPath: FieldPath,
      detail: String,
  ...
  /** Creates a configuration error for a missing value.
    ...
    *   the path to the field
    * @param annotations
    *   the annotations that were tried
    * @return
    *   a ConfigError with a Missing reason
    */
  def missing(
      fieldPath: FieldPath,
      annotations: Seq[ConfigAnnotation]
  ...
  /** Creates a configuration error for other types of errors.
    ...
    *   details about the error
    * @return
    *   a ConfigError with an Other reason
    */
