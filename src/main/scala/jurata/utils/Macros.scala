...
private[jurata] object Macros {
  ...
  def filterAnnotation(using quotes: Quotes)(a: quotes.reflect.Term): Boolean =
    ...
    scala.util.Try(a.tpe <:< TypeRepr.of[jurata.ConfigAnnotation]).toOption.contains(true)

  def exprOfOption[T: Type](using quotes: Quotes)(optExpr: Option[Expr[T]]): Expr[Option[T]] =
    optExpr match
      case Some(e) => '{ Some($e) }
      case None    => '{ None }

  def fieldMetadataImpl[A: Type](using quotes: Quotes): Expr[Map[String, FieldMetadata]] = 
    import quotes.reflect.*
    ...
    def default(fieldIdx: Int): Option[Expr[Any]] = 
      tpe.typeSymbol.companionClass
        ...
        .map { defaultMethod =>
          val callDefault = {
            val base = Ident(tpe.typeSymbol.companionModule.termRef).select(defaultMethod)
            ...
            Expr(field.name) -> {
              ...
              val annotations = Expr.ofList(
                field
                  ...
                  .map(_.asExpr.asExprOf[jurata.ConfigAnnotation]).reverse
              )
  ...
  def typeMetadataImpl[A: Type](using quotes:Quotes): Expr[TypeMetadata[A]] = 
    ...
    val valuesMethod = typeSymbol.companionModule.methodMember("values").headOption
