...
class FallbacksSpec extends AnyFlatSpec with Matchers with EitherValues {
  ...
  it should "don't use fallback value if first is available" in {
    ...
    //given
    case class Config(@prop("port") @env("PORT") @env("FALLBACK_PORT") port: Int) derives ConfigValue

    //when
    val configWithEnv = load[Config](
      using reader = ConfigReader.mocked
        .onEnv("PORT", "1000")
    ...
    //when
    val configWithProp = load[Config](
      using reader = ConfigReader.mocked
        .onEnv("PORT", "1000")
        .onEnv("FALLBACK_PORT", "1001")
        .onProp("port", "9999")
  ...
  it should "use fallback value if first is not available" in {
    ...
    //given
    case class Config(@env("PORT") @env("FALLBACK_PORT") port: Int)derives ConfigValue
